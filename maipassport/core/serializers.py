...
class OtpSendSerializer(serializers.Serializer):
    ...
    def validate(self, validated_data):
            ...
            if 'phone' in validated_data:
                ...
                if validated_data['otp_type'] == 'NEW':
                    ...
                    try:
                        ...
                        phone = AppUserServices.check_phone(validated_data['phone'])
                        ...
                        logger_writer('SYSTEM', 'error', 'VALIDATE', f'{validated_data["phone"]} = {phone}')
                else:
                    try:
                        # phone_dict = AppUserServices.change_phone(validated_data['phone'])
                        phone = AppUserServices.check_phone(validated_data['phone'])
        ...
        if 'account' in validated_data:
            if User.objects.filter(username=validated_data['account'], role__name=Role.ADMIN).exists():
                raise AccountCantReset

            app_user = AppUser.objects.select_related('auth_user').filter(
                auth_user__username=validated_data['account'])
            logger_writer('SYSTEM', 'info', 'VALIDATE', f'app_user: {app_user}')
            if not app_user.exists():
                raise UserNotExists
            else:
                    ...
                    if validated_data['otp_type'] == 'NEW':
                        ...
                        try:
                            ...
                            if AppUser.objects.filter(phone=phone).exists():
                                raise PhoneAlreadyExists
                            validated_data['phone'] = phone
                            logger_writer('SYSTEM', 'error', 'VALIDATE',
                                          f'{validated_data["phone"]} = {phone}')
                    else:
                        try:
                            # phone_dict = AppUserServices.change_phone(validated_data['phone'])
                            phone = AppUserServices.check_phone(validated_data['phone'])

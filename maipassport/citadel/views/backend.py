...
from maipassport.records.models import ApproachRecord, PlaceEntryRecord, NTPCCalender
...
@login_required4
def place_checkin_open(request, pub_id, status):
    company = Company.objects.filter(pub_id=pub_id)
    if company.exists():
        company = company.first()
        if status == 'ON':
            company.place_checkin = True
        else:
            company.place_create = False
        company.save()
    return HttpResponseRedirect(reverse('company_detail', kwargs={'pub_id': pub_id}))
...
def place_management(request):
    ...
    if place_address:
        place_list = place_list.filter(location__location_address=place_address)
...
def checkin_tract(request):
    ...
    if company_name:
        ...
        return_data['sel_company_flag'] = company_name
        place_entry_list = place_entry_list.filter(app_user__usercompanytable__company__name=company_name).order_by('-created')
    else:
        request.session['company_name'] = ''
        return_data['sel_company_flag'] = ''

    if user_name:
        return_data['sel_type'] = 'user'
        request.session['user_name'] = user_name
        place_entry_list = place_entry_list.filter(app_user__auth_user__first_name=user_name)
    else:
        request.session['user_name'] = ''

    if created_at_min:
        request.session['created_at_min'] = created_at_min
        logger_writer('SYSTEM', 'info', 'CHECKIN TRACK', f'create_at_min' + created_at_min)
        try:
            create_at_min = datetime.strptime(created_at_min, '%Y-%m-%d')
        except (TypeError, ValueError) as e:
            pass
        else:
            place_entry_list = place_entry_list.filter(created__gte=timezone.make_aware(create_at_min))
    ...
    if created_at_max:
        ...
        logger_writer('SYSTEM', 'info', 'CHECKIN TRACK', f'create_at_max' + created_at_max)
        try:
            # Plus one day to cover whole day. Ex: 2018-10-10T00:00:00 -> 2018-10-11T00:00:00
            # and then we use less than expression to query
            create_at_max = datetime.strptime(created_at_max, '%Y-%m-%d') + timedelta(days=1)
        except (TypeError, ValueError) as e:
            pass
        else:
            place_entry_list = place_entry_list.filter(created__lt=timezone.make_aware(create_at_max))
    ...
    company_list = Company.objects.all()
    for company in company_list:
        user_dict[company.name] = [user_first_name.app_user.auth_user.first_name for user_first_name in company.usercompanytable_set.all()]
    user_dict['無'] = [user_first_name.auth_user.first_name for user_first_name in AppUser.objects.all()]
    return_data['place_entry_list'] = place_entry_list
    return_data['page_range'] = page_range
    return_data['company_list'] = Company.objects.all()
    return_data['user_list'] = AppUser.objects.all()
    return_data['place_list'] = Place.objects.all()
    return_data['user_dict'] = user_dict
    return render(request, 'backend/checkin_track.html', context=return_data)
...
def checkin_export(request):
    ...
    # place_entry_list = PlaceEntryRecord.objects.filter(app_user__usercompanytable__company__name=company_name)
    """
    check if company_name, user_name, time period are selected by user
    add company_name(if selected), user_name(if selected) to filename
    filter place_entry_list with company_name, user_name, time_period
    """
    if company_name:
        place_entry_list = place_entry_list.filter(app_user__usercompanytable__company__name=company_name)
    ...
    file_name += '.xls'
    """
    reformat place_entry_list into
    {
    "day" => created
    "app_user_name" => name
    "max" => checkout
    ...
    """
    format_list = place_entry_list.extra(select={'day': "TO_CHAR(records_placeentryrecord.created, 'YYYY-MM-DD')"}).\
        values('app_user__auth_user__first_name', 'day').annotate(Max('created'), Min('created'))
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = "attachment; filename*=UTF-8''{}".format(escape_uri_path(file_name))

    wb = xlwt.Workbook(encoding='utf-8')
    # Alan_todo: ws = wb.add_sheet (需再不同使用者時增加)
    ...
    out_format = xlwt.XFStyle()
    out_format.pattern = pattern2
    out_format.borders = borders2
    out_format.num_format_str = '[h]:mm;@'

    time_format = xlwt.XFStyle()
    time_format.pattern = pattern2
    time_format.borders = borders2
    ...
    col_count = {}
    user_list = place_entry_list.distinct('app_user')

    # create all sheet base on username
    # Alan_todo: original excel column
    # for user in user_list:
    #     col_count[user.app_user.auth_user.first_name] = 1
    #     ws = wb.add_sheet(user.app_user.auth_user.first_name)
    #     ws.write(0, 0, str('日期'), style)
    #     ws.write(0, 1, str('地點'), style)
    #     ws.write(0, 2, str('使用者帳號'), style)
    #     ws.write(0, 3, str('接觸者公司'), style)
    #     ws.write(0, 4, str('紀錄'), style)
    # Alan_todo: v2 excel column
    """
    create sheets for each user from selected user list
    row format
    if time_period is set: 
        [company_name date至date出勤紀錄]
        [員工姓名： {name}]
        [日期 ｜ 上班 ｜ 下班 ｜ 出勤總工時 ｜ 備註]
    """
    for user in user_list:
        col_count[user.app_user.auth_user.first_name] = 2
        if(get_sheet_by_name(wb, user.app_user.auth_user.first_name)):
            combine_string = user.app_user.auth_user.first_name + "(" + user.app_user.auth_user.username + ")"
            ws = wb.add_sheet(combine_string, cell_overwrite_ok=True)
        else:
            ws = wb.add_sheet(user.app_user.auth_user.first_name, cell_overwrite_ok=True)
        if created_at_min and created_at_max:
            ws.write(0, 0, str('%s %s至%s出勤紀錄' % (company_name, datetime.strptime(created_at_min, '%Y-%m-%d').date(), datetime.strptime(created_at_max, '%Y-%m-%d').date())), style2)
            ws.write(1, 0, str('員工姓名： %s' % (user.app_user.auth_user.first_name)), style2)
        else:
            ws.write(0, 0, str('%s 出勤紀錄' % (company_name)), style2)
            ws.write(1, 0, str('員工姓名： %s' % (user.app_user.auth_user.first_name)), style2)

        ws.write(col_count[user.app_user.auth_user.first_name], 0, str('日期'), style)
        ws.write(col_count[user.app_user.auth_user.first_name], 1, str('上班'), style)
        ws.write(col_count[user.app_user.auth_user.first_name], 2, str('下班'), style)
        ws.write(col_count[user.app_user.auth_user.first_name], 3, str('出勤總工時'), style)
        ws.write(col_count[user.app_user.auth_user.first_name], 4, str('備註'), style)
        col_count[user.app_user.auth_user.first_name] += 1

    # Alan_todo: group by with autofil blank date v2
    """
    check if time_period selected
    if selected, should fill out blank if user forget to checkin and checkout
    """
    if created_at_min and created_at_max:
        out_dict = {}
        for record in format_list:
            if record['app_user__auth_user__first_name'] not in out_dict:
                out_dict[record['app_user__auth_user__first_name']] = {}
            out_dict[record['app_user__auth_user__first_name']][record['day']] = {
                'created__min': record['created__min'],
                'created__max': record['created__max']}
        created_min = datetime.strptime(created_at_min, "%Y-%m-%d")
        created_max = datetime.strptime(created_at_max, "%Y-%m-%d") + timedelta(days=1)
        for each_user in out_dict:
            ws = wb.get_sheet(each_user)
            for single_date in daterange(created_min, created_max):
                check_holiday = NTPCCalender.objects.filter(date=datetime.strftime(single_date, '%Y-%m-%d'))
                if not check_holiday.exists() or (check_holiday.exists() and check_holiday.first().isHoliday == False):
                    if single_date.strftime("%Y-%m-%d") in out_dict[each_user]:
                        ws.write(col_count[each_user], 0, str(single_date.strftime("%Y-%m-%d")), style2)
                        ws.write(col_count[each_user], 1, utc_time_to_local_time(out_dict[each_user][single_date.strftime("%Y-%m-%d")]['created__min']).strftime("%I:%M:%S %p"), time_format)
                        if out_dict[each_user][single_date.strftime("%Y-%m-%d")]['created__min'] != out_dict[each_user][single_date.strftime("%Y-%m-%d")]['created__max']:
                            ws.write(col_count[each_user], 2, utc_time_to_local_time(out_dict[each_user][single_date.strftime("%Y-%m-%d")]['created__max']).strftime("%I:%M:%S %p"), time_format)
                        else:
                            ws.write(col_count[each_user], 2, '', style2)

                        if(out_dict[each_user][single_date.strftime("%Y-%m-%d")]['created__max'] == out_dict[each_user][single_date.strftime("%Y-%m-%d")]['created__min']):
                            ws.write(col_count[each_user], 3, '', style2)
                        else:
                            formula_max = 'C' + str(col_count[each_user]+1)
                            formula_min = 'B' + str(col_count[each_user]+1)
                            ws.write(col_count[each_user], 3, Formula("TIMEVALUE(%s)-TIMEVALUE(%s)-IF(AND(TIMEVALUE(%s)<12/24,TIMEVALUE(%s)>13/24),1/24,0)" % (formula_max, formula_min, formula_min, formula_max)), out_format)
                        ws.write(col_count[each_user], 4, '', style2)
                    else:
                        ws.write(col_count[each_user], 0, str(single_date.strftime("%Y-%m-%d")), style2)
                        ws.write(col_count[each_user], 1, '', style2)
                        ws.write(col_count[each_user], 2, '', style2)
                        ws.write(col_count[each_user], 3, '', style2)
                        ws.write(col_count[each_user], 4, '', style2)

                    col_count[each_user] += 1
            ws.write_merge(col_count[each_user], col_count[each_user], 0, 2, "合計時數", style3)
            ws.write(col_count[each_user], 3, Formula("SUM(D4:D%s)" % (col_count[each_user])),
                     out_format)
    else:
        """
        if time_period not set
        print out only days that have checkin records
        """
        for record in format_list:
            record_app_user = record['app_user__auth_user__first_name']

            ws = wb.get_sheet(record_app_user)
            ws.write(col_count[record_app_user], 0, str(record['day']), style2)
            ws.write(col_count[record_app_user], 1, utc_time_to_local_time(record['created__min']).strftime("%I:%M:%S %p"),
                     time_format)
            if (record['created__min'] != record['created__max']):
                ws.write(col_count[record_app_user], 2,
                         utc_time_to_local_time(record['created__max']).strftime("%I:%M:%S %p"), time_format)
            else:
                ws.write(col_count[record_app_user], 2, '', style2)

            if (record['created__max'] == record['created__min']):
                ws.write(col_count[record_app_user], 3, '', style2)
            else:
                formula_max = 'C' + str(col_count[record_app_user] + 1)
                formula_min = 'B' + str(col_count[record_app_user] + 1)
                ws.write(col_count[record_app_user], 3, Formula("TIMEVALUE(%s)-TIMEVALUE(%s)-IF(AND(TIMEVALUE(%s)<12/24,TIMEVALUE(%s)>13/24),1/24,0)" % (formula_max, formula_min, formula_min, formula_max)), out_format)
            ws.write(col_count[record_app_user], 4, '', style2)
            col_count[record_app_user] += 1
        """
        at the end of each sheet, added 
        [合計時數 ： {total_hour}]
        """
        for record in format_list:
            record_app_user = record['app_user__auth_user__first_name']
            ws = wb.get_sheet(record_app_user)
            ws.write_merge(col_count[record_app_user], col_count[record_app_user],  0, 2, "合計時數", style3)
            ws.write(col_count[record_app_user], 3, Formula("SUM(D4:D%s)" % (col_count[record_app_user])), out_format)
...
def company_place_tract_export(request, pub_id):
    ...
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = "attachment; filename*=UTF-8''{}".format(escape_uri_path('地點追蹤清單.xls'))

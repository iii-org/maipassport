...
from maipassport.records.models import (QuestionnaireField, Questionnaire, ApproachRecord, AttendanceRecord,
                                        PlaceEntryRecord, HealthRecord, VisitorRegistration, VacRecord, RapidTestRecord,
                                        CovidHistoryRecord)
from maipassport.records.services import GpsService
from maipassport.records.forms import UploadVacImage, UploadCovidImage, UploadRapidImage
...
@login_required2
def health_check(request, user_id, token):
    return_data = dict()
    # form = UploadVacImage()

    # if request.method == 'POST':
    #     sel_type = request.POST.get("sel_type")
    #     form = UploadVacImage(request.POST, request.FILES)
    #     if form.is_valid():
    #         uploaded_img = form.save(commit=False)
    #         uploaded_img.image_data = form.cleaned_data['image'].file.read()
    #         uploaded_img.save()
    #         return render(request, 'app_web_n/health_check.html', {'form': form})
    #
    #     else:
    #         form = UploadVacImage()
    #     return render(request, 'app_web_n/health_check.html', {'form': form})
    ...
    seven_day_ago = stime - timedelta(days=7)
    # all record
    vac_record = VacRecord.objects.filter(app_user=request.app_user).order_by('-created')
    vac_record_list = list()
    # for record in vac_record:
    ...
    # return_data['health_question'] = health_question
    return_data['approach_record'] = approach_record
    return render(request, 'app_web_n/health_check.html', context=return_data)
...
@login_required2
def upload_health_img(request, user_id, token):
    user_img_file = request.FILES.copy()
    sel_type = request.POST.get('sel_type')
    if 'upload_img' in user_img_file:
        img_file_obj = user_img_file['upload_img']
        now = datetime.now()
        file_path = f'media/{user_id}/health_image/{now.strftime("%Y%m%d%H%M%S")}.{img_file_obj.name.split(".")[1]}'
        url = AppUserServices.upload_to_aws(file_path, user_img_file['upload_img'])
        if sel_type == "VAC_RECORD":
            VacRecord.objects.create(app_user=request.app_user, image_url=url)
        elif sel_type == "RAP_TEST":
            RapidTestRecord.objects.create(app_user=request.user.appuser, image_url=url)
        elif sel_type == "COV_HISTORY":
            CovidHistoryRecord.objects.create(app_user=request.user.appuser, image_url=url)
    return HttpResponseRedirect(
        reverse('health_check', kwargs={'user_id': request.user.appuser.pub_id,
                                        'token': request.user.appuser.api_token}))

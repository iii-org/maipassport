...
# calender
class NTPCCalender(models.Model):
    pub_id = AutoPubIDField(
        _('Public Id'),
        db_index=True
    )
...
# Vaccine Record need to store uploader(app_user), upload_time(CreatedAndModifiedMixin), uploaded_img(to_be_added)
class VacRecord(CreatedAndModifiedMixin):
    pub_id = AutoPubIDField(
        _('Public Id'),
        db_index=True
    )
    ...
    image = models.FileField(validators=[FileExtensionValidator(allowed_extensions=['png', 'jpeg'])])
    image_data = models.BinaryField(null=True)


# RapidTestRecord: need to store uploader(app_user), upload_time(CreatedAndModifiedMixin), uploaded_img(to_be_added)
class RapidTestRecord(CreatedAndModifiedMixin):
    pub_id = AutoPubIDField(
        _('Public Id'),
        db_index=True
    )
...
# CovidHistoryRecord: need to store uploader(app_user), upload_time(CreatedAndModifiedMixin), uploaded_img(to_be_added)
class CovidHistoryRecord(CreatedAndModifiedMixin):
    pub_id = AutoPubIDField(
        _('Public Id'),
        db_index=True
    )

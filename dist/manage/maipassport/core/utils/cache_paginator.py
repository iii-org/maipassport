...
class CachedPaginator(Paginator):
    ...
    def page_range_list(self, page):
        if not page or page == '':
            page = 1
        else:
            page = int(page)
        max_pages_count = len(self.page_range)
        max_pages_of_control = 10
        base_compu = max(int(page / max_pages_of_control) * max_pages_of_control, 0)
        start_index_page = base_compu - 1
        end_index_page = start_index_page + max_pages_of_control

        if start_index_page < 0:
            start_index_page = 0

        if max_pages_count < end_index_page:
            end_index_page = max_pages_count

        return list(self.page_range)[start_index_page:end_index_page]

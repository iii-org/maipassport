...
def liff_router(request):
    ...
    return_data = return_data_base.copy()
...
def liff_sign_up(request):
    ...
    if request.method == 'POST':
        ...
        if account and phone and user_real_name and password and chk_password:
            try:
                ...
                elif AppUser.objects.filter(phone=phone_dict['fix_phone']).exists():
                    # TODO: 電話已存在
...
def liff_sign_up_otp(request):
    ...
    if request.method == 'POST':
        ...
        if otp and otp_id:
            try:
                OtpService(otp_id).verify(otp, verify_phone=user_sign_up_data['fix_phone'])
            except Exception as e:
                return_data['message'] = _('Verify Otp Error, Please resend Otp')
                # return render(request, 'app_web_n/sign_up_otc.html', context=return_data)
            else:
                try:
                    app_user = AppUserServices.create_app_user(user_account=user_sign_up_data['account'],
                    ...
                    app_user.phone = user_sign_up_data['fix_phone']
